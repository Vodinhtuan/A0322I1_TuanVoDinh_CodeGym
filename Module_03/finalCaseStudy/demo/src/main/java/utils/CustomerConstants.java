...
public interface CustomerConstants {
    ...
    String UPDATE_CUSTOMER = "Update customer set customer_type_id = ?, customer_name = ?, customer_birthday = ?, customer_gender = ?, customer_id_card = ?," +
            " customer_phone = ?, customer_email = ?, customer_address = ? where id = ?";
    String FIND_CUSTOMER_BY_ID = "select * from customer where customer_id = ?";
    String DELETE_BY_ID = "DELETE FROM customer WHERE customer_type_id = ?";
